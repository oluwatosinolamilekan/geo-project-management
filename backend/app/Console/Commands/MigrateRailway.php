...
class MigrateRailway extends Command
{
    protected $signature = 'migrate:railway {--fresh : Run fresh migrations} {--seed : Run seeders after migration}';
    protected $description = 'Run migrations optimized for Railway PostgreSQL (no superuser privileges)';

    public function handle()
    {
        $this->info('🚂 Starting Railway-optimized migrations...');
        ...
        try {
            ...
            // Handle fresh migrations more carefully
            if ($this->option('fresh')) {
                $this->info('🧹 Preparing for fresh migration...');
                $this->prepareFreshMigration();
            }

            // Run migrations using standard Laravel approach
        ...
        $this->info('🎉 Railway migration completed successfully!');
        return 0;
    }
    ...
    private function prepareFreshMigration()
    {
        try {
            ...
            // Get all table names except migrations, ordered by dependencies
            ...
            $this->info('🗑️ Dropping existing tables in dependency order...');
    ...
    {
        // Define table dependencies (child tables first, then parent tables)
        $dependencyOrder = [
            'pins',           // Depends on projects
            'projects',       // Depends on regions  
        ...
        // Get existing tables and sort them by dependency order
        ...
        // Add any remaining tables not in our dependency list
        foreach ($existingTables as $table) {
            if (!in_array($table->tablename, array_column($orderedTables, 'tablename'))) {
                $orderedTables[] = $table;
            }
        }
    ...
    private function runMigrations()
    {
        try {
            ...
            $this->info('🔍 Checking existing migrations...');
            $runMigrations = $this->getRunMigrations();
            
            // Run migrations with force flag, but skip existing ones
            ...
            // Try alternative approach - run migrations one by one in dependency order
